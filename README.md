...
[GitHub Repository Branch](https://github.com/WarrenUhrich/lighthouse-labs-sql-from-our-apps/tree/2024.03.06-web-immersive-national-5february2024) | [Vimeo Video Recording](https://vimeo.com/920191571/9c013d172e?share=copy)
...
If you haven't already, make sure you run `npm install` to make sure you have the necessary dependencies.

Run any of the following to execute CRUD operations via the terminal:

* `node movie-villains help # Display available commands`
* `node movie-villains index # List all villains`
* `node movie-villains show <ID> # Show single villain by ID`
* `node movie-villains create <NAME> <MOVIE> # Create a new villain`
* `node movie-villains update <ID> <NAME> <MOVIE>`
* `node movie-villains delete <ID> # Delete villain by ID`

### Web Application

1. `npm install`
2. Fill in the `.env` file if you have not already; create a copy of the `.env.example` file to more easily follow the format.
3. `node express-server`
4. [Visit the Villains Index](http://localhost:8181/movie-villains) (default: http://localhost:8181/movie-villains)
4. [Visit the Villains Show](http://localhost:8181/movie-villains/1) (default: http://localhost:8181/movie-villains/1)
...
```bash
node movie-villains index` # List all villains.
node movie-villains show <id>` # Show specific villain.
node movie-villains edit <id> <name> <movie>` # Edit specific villain.
node movie-villains create <name> <movie>` # New villain.
node movie-villains delete <id>` # Remove specific villain.
```
